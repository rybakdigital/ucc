...
/**
 * Ucc\Data\Types\Basic\CountryType
 * Defines CountryType data
 *
 * @author Kris Rybak <kris@krisrybak.com>
 */
class CountryType implements TypeInterface
{
    public static $requirementsOptions = array(
        'code'   => 'Must be a country code, i.e. GB',  // Example: GB
    );
    ...
    public static function check($value, array $requirements = array())
    {
        ...
        if (isset($requirements['code'])) {
            if (!in_array($requirements['code'], self::countryCodeTypes())) {
                throw new InvalidDataException("supported code types are " . implode(', ', self::countryCodeTypes()));
            }

            if (!array_key_exists($value, self::getCountries($requirements['code']))) {
                throw new InvalidDataTypeException("value must be a vaild country code type " . $requirements['code']);
            }
        } else {
            $type = 'name';
            if (!in_array($value, $countries)) {
                throw new InvalidDataTypeException("value must be a vaild country name");
            }
        }
     ...
     * @return array
     */
    public static function getCountries($type = 'alpha2') {
        $ret = array();

        foreach (self::$countries as $key => $country) {
            $ret[$country[$type]] = $country['name'];
        }
